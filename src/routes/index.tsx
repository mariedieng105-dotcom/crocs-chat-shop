    ...
    <div className="min-h-screen bg-background">
      ...
      <section id="accueil" className="overflow-hidden border-b border-border">
        <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 py-12 md:grid-cols-2 md:py-16">
          <div className="min-w-0 space-y-5"><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-turquoise"><span className="h-1.5 w-1.5 rounded-full bg-turquoise" />Dakar, Sénégal</span><EditableText name="heroTitle" as="h1" className="break-words font-display text-3xl leading-[1.05] tracking-tight sm:text-5xl md:text-6xl" /><EditableText name="heroSlogan" className="max-w-md text-base leading-relaxed text-muted-foreground sm:text-lg" /><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><Button asChild size="lg" className="rounded-full bg-orange-pop px-7 text-primary-foreground shadow-lg shadow-orange-pop/30 transition hover:bg-orange-pop/90 hover:shadow-xl hover:shadow-orange-pop/40"><a href="#boutique" className="inline-flex items-center gap-2">{texts["heroCta"]}<ArrowRight className="h-4 w-4" /></a></Button><Button asChild size="lg" variant="outline" className="rounded-full border-2 px-7 shadow-sm transition hover:bg-secondary"><a href={waLink("Bonjour Diaby Store, je veux des infos sur vos Crocs.")} target="_blank" rel="noreferrer">Nous écrire</a></Button></div></div>
          <img src={resolveImageUrl(logoAsset.url)} alt="CROCS_DAKAR_221 — Diaby Store" width={1280} height={697} className="w-full rounded-2xl border border-border object-cover shadow-xl" />
        </div>
      </section>
